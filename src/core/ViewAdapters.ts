...
export class MarkdownViewAdapter implements ViewAdapter {
  ...
  adapt(note: Note): { markdown: string; html?: string } {
    ...
    switch (note.type) {
      ...
      case 'code':
        return { markdown: `\`\`\`${n.language || 'text'}\n${n.code || ''}\n\`\`\`` };
...
export class RichTextViewAdapter implements ViewAdapter {
  ...
  adapt(note: Note): { html: string; content?: any } {
    ...
    switch (note.type) {
      ...
      case 'code':
        return { html: `<pre><code>${this.escapeHtml(n.code || '')}</code></pre>` };
      
      case 'image':
        if (n.url) {
          return { html: `<img src="${n.url}" alt="${this.escapeHtml(n.alt || '')}" />` };
        }
        return { html: `<p><em>Image: ${this.escapeHtml(n.alt || 'No URL')}</em></p>` };
      
      case 'smart-layer':
        return { html: `<p>${this.escapeHtml(n.source?.data || '')}</p>` };
      
      default:
        return { html: `<pre>${this.escapeHtml(JSON.stringify(note, null, 2))}</pre>` };
    }
  }
...
export class ImageViewAdapter implements ViewAdapter {
  ...
  adapt(note: Note): { url?: string; alt?: string; width?: number; height?: number; fallbackText?: string } {
    const n = note as any;
  ...
  private extractText(note: Note): string {
    ...
    switch (note.type) {
      ...
      case 'markdown': return n.markdown || '';
      case 'code': return n.code || '';
      case 'rich-text': return n.html || '';
      case 'smart-layer': return n.source?.data || '';
      default: return '';
    }
  }
}
...
export class SmartLayerViewAdapter implements ViewAdapter {
  ...
  adapt(note: Note): { source: { type: string; data: any }; layers: any[]; activeLayerId?: string } {
    const n = note as any;
...
class AdapterRegistry {
  ...
  adaptNote(note: Note, viewAs: NoteType | 'json'): { data: any; warning: string | null } {
