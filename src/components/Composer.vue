  ...
  <form class="floating-surface relative flex flex-col gap-3 p-3 md:p-4" @submit.prevent="send">
    <div class="flex items-center justify-between gap-3">
      <div class="flex items-center gap-2">
        <div v-for="action in actions" :key="action.id" class="relative">
          ...
          >
            ...
            <span v-else class="text-[0.6rem] font-semibold uppercase tracking-wide">{{ action.label }}</span>
          </button>
      ...
      <div class="flex items-center gap-3">
        <span v-if="isTyping" class="text-[0.7rem] uppercase tracking-wide text-ink/60">{{ t('writing') }}</span>
        <button
          type="button"
