...
export function detectFormat(content: string): ChordFormat {
  ...
  if (/\|[^|]*[A-G][#b]?[^|]*\|/.test(content) || /^#\s+.+/m.test(content))
    return "tab";
  return "inline";
}
  ...
  if (format === "chordpro") {
    ...
    for (const match of matches) {
      ...
      const chordMatches = line.matchAll(
        /([A-G][#b]?(?:m|maj|dim|aug|sus|add|[0-9])*)/g
      );
    ...
    return content.replace(
      ...
      (_match, chord: string) => {
        ...
        const transposed = parts
          .map((c: string) => transposeChord(c.trim(), semitones))
          .join("/");
        return `[${transposed}]`;
      }
    );
