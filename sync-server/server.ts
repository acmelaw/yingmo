  ...
  if (!room) {
    ...
    doc.on('update', (update: Uint8Array) => {
      ...
      if (USE_REDIS && redisClient) {
        redisClient.publish(`yjs:${roomName}:update`, Buffer.from(update).toString('base64'))
      }
    })
...
function handleConnection(conn: WebSocket, req: http.IncomingMessage) {
  ...
  console.log(`Client connected to room: ${roomName} (${room.connections.size} total)`)
}

/**
 * Handle sync protocol messages
 */
function handleSyncMessage(decoder: decoding.Decoder, room: Room, sender: WebSocket) {
  const encoder = encoding.createEncoder()
  const syncMessageType = syncProtocol.readSyncMessage(decoder, encoder, room.doc, sender)
 ...
 */
function handleAwarenessMessage(decoder: decoding.Decoder, room: Room, sender: WebSocket) {
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/metrics') {
    const metrics = [
      ...
      `yjs_connections_total ${Array.from(rooms.values()).reduce((sum, room) => sum + room.connections.size, 0)}`,
      ``,
...
║  Persist:   ${PERSISTENCE_ENABLED ? 'Enabled' : 'Disabled'}                               ║
╚════════════════════════════════════════════════════╝
    `)
  })
}
