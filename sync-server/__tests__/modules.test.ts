...
import type { BaseNote, TextNote, RichTextNote, MarkdownNote, CodeNote } from '../types/note.js';
...
describe('Note Type Modules', () => {
  ...
  describe('Text Note Module', () => {
    ...
    it('should serialize and deserialize', async () => {
      ...
      const note = await handler!.create({ text: 'Test', editHistory: [] }, baseNote);
  ...
  describe('Rich Text Note Module', () => {
    ...
    it('should validate rich text note', async () => {
      ...
      const note = await handler!.create({ content: { type: 'doc', content: [] } }, baseNote);
      expect(handler!.validate(note)).toBe(true);
    });
  });
