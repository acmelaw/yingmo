...
describe("NoteService Integration Tests", () => {
  ...
  describe("CRUD Operations", () => {
    ...
    it("should filter notes by type", async () => {
      ...
      const textNotes = await noteService.list("tenant1", "user1", {
        type: "text",
      });
      const mdNotes = await noteService.list("tenant1", "user1", {
        type: "markdown",
      });
    ...
    it("should filter notes by archived status", async () => {
      await noteService.create(
        "text",
        { text: "Active", archived: false },
        "tenant1",
        "user1"
      );
      await noteService.create(
        "text",
        { text: "Archived", archived: true },
        "tenant1",
        "user1"
      );

      const activeNotes = await noteService.list("tenant1", "user1", {
        archived: false,
      });
      const archivedNotes = await noteService.list("tenant1", "user1", {
        archived: true,
      });
